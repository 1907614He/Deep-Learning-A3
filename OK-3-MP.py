...
df = df.dropna()#删除缺失值
# 这里我们只使用收盘价作为特征
data = df[['close','open','vol']]
# 数据归一化
scaler = MinMaxScaler(feature_range=(-1, 1))
scaler_ = MinMaxScaler(feature_range=(-1, 1))
data['close'] = scaler.fit_transform(data['close'].values.reshape(-1,1))
data['open'] = scaler_.fit_transform(data['open'].values.reshape(-1,1))
data['vol'] = scaler_.fit_transform(data['vol'].values.reshape(-1,1))
data=data.values
...
x, y = create_dataset(training_set, sequence_length)
...
# 实例化模型
model = RNN(input_size=3, hidden_size=100, num_layers=2, output_size=1)
...
plt.title('Multi-featured: Train and Validation loss')
plt.legend()
plt.show()
...
plt.plot(data_predict, label='Multi-featured: Predicted Data')
plt.legend()
plt.show()
